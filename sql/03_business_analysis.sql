...
-- 3. Customer Analysis
-- Revenue by gender
SELECT c.gender, SUM(s.revenue) as revenue FROM sales s JOIN customers c ON s.customer_id=c.customer_id GROUP BY c.gender ORDER BY revenue DESC;
-- Revenue by loyalty membership
SELECT c.loyalty_member, SUM(s.revenue) as revenue FROM sales s JOIN customers c ON s.customer_id=c.customer_id GROUP BY c.loyalty_member;
...
SELECT CASE c.month
    ...
    END AS month, 
    SUM(s.revenue) as revenue FROM sales s JOIN calendar c ON s.order_date=c.date GROUP BY month;
